...
def feature_align_superpoint(raw_feature: Tensor, P: Tensor, ns_t: Tensor, ori_size: tuple, device=None):
    """
    raw_feature: feature map from superpoint
    P: tensor contains point, shape [batch_num, point_num, point_dim]
    ns_t:tensor contains point numbers
    ori_size:original image size
    return F, tensor contains point features
    """

    ##some errors wigh grid sample
    # H,W = ori_size
    # if device is None:
    ...
    #
    # #batch operation
    # W_H = torch.tensor((ori_size[1],ori_size[0]), device=device)
    # _P = P / (W_H/2.) - 1.
    # _P = _P.contiguous().view(batch_num,1,-1,2)
    #
    # desc = torch.nn.functional.grid_sample(raw_feature, _P, align_corners=True)
    # desc = desc.reshape(batch_num, channel_num, -1)
    # desc_norm = torch.norm(desc,dim=1).view(batch_num,-1)
    # desc_norm = desc_norm.repeat(1, desc.shape[1]).view(batch_num, desc.shape[1], -1)
    # desc = desc / desc_norm
    #
    # return desc#[batch_num, channel_num, n_max]
